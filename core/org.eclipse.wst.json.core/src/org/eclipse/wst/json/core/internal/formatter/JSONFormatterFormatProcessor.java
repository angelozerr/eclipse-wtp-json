...
/**
 * This is a wrapper for the new JSON formatter so that it can be still
 * considered an IStruturedFormatProcessor
 */
public class JSONFormatterFormatProcessor implements IStructuredFormatProcessor {
	private DefaultJSONPartitionFormatter fFormatter = null;
	...
	public void formatFile(IFile file) throws IOException, CoreException {
		...
		try {
			...
			if (structuredModel != null && !structuredModel.isSharedForEdit()
	...
	public void formatModel(IStructuredModel structuredModel, int start,
		...
		DocumentRewriteSession activeRewriteSession = ((IDocumentExtension4) structuredDocument)
				.getActiveRewriteSession();
		boolean startedRewriteSession = false;
		if (activeRewriteSession == null) {
			activeRewriteSession = ((IDocumentExtension4) structuredDocument)
		...
		try {
			...
			if (startedRewriteSession && activeRewriteSession != null) {
				((IDocumentExtension4) structuredDocument)
						.stopRewriteSession(activeRewriteSession);
			}
	...
	public void formatNode(Node node) {
		if (node instanceof IJSONNode) {
			IJSONNode domNode = (IJSONNode) node;
			formatModel(domNode.getModel(), domNode.getStartOffset(),
					domNode.getLength());
		}
	}
	...
	private DefaultJSONPartitionFormatter getFormatter() {
		if (fFormatter == null) {
			fFormatter = new DefaultJSONPartitionFormatter();
		}
		return fFormatter;
	}
