 ...
 */
public abstract class JSONStructuredDocumentRegionContainer extends
		JSONNodeImpl {
	...
	/**
	 * JSONContainer constructor comment. currently public but may be made
	 * default access protected in future.
	 */
	public JSONStructuredDocumentRegionContainer() {
		super();
	}
	 ...
	 */
	public JSONStructuredDocumentRegionContainer(
			JSONStructuredDocumentRegionContainer that) {
		super(that);
	}
	...
	boolean includeRangeStructuredDocumentRegion(
		...
		if (first != null) {
			if (getFirstStructuredDocumentRegion() == null
					|| getFirstStructuredDocumentRegion().getStart() > first
							.getStart()) {
		...
		if (last != null) {
			if (getLastStructuredDocumentRegion() == null
					|| getLastStructuredDocumentRegion().getStart() < last
							.getStart()) {
	...
	public boolean propagateRangeStructuredDocumentRegion() {
		...
		JSONStructuredDocumentRegionContainer parent = (JSONStructuredDocumentRegionContainer) getParentNode();
		if (parent == null)
			return bModified;

		boolean b = parent.includeRangeStructuredDocumentRegion(
	...
	IStructuredDocumentRegion setFirstStructuredDocumentRegion(
			IStructuredDocumentRegion node) {
		firstStructuredDocumentRegion = node;
		return node;
	}

	IStructuredDocumentRegion setLastStructuredDocumentRegion(
			IStructuredDocumentRegion node) {
		lastStructuredDocumentRegion = node;
		return node;
	}

	public void setRangeStructuredDocumentRegion(
	...
	private boolean validateRange() {
		...
		if (firstStructuredDocumentRegion != null
			...
			if (this.firstStructuredDocumentRegion == null) {
				...
				bModified = true;
			} else if (this.firstStructuredDocumentRegion.getStart() > this.lastStructuredDocumentRegion
					.getStart()) {
